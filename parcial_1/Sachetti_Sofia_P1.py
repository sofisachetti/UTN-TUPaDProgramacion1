...
while control_menu:
    ...
    if opcion == "1":
        ...
        if not herramientas: # Si la lista de herramientas esta vacia imprimo el mensaje en la consola
            ...
            agotados = [] # lista para almacenar los productos agotados
            
            for i in range(len(existencias)): # Recorro la lista de existencias con un bucle for
                if existencias[i] == 0: # Si encuentro alguna existencia en 0 lo alamceno en la lista
                    agotados.append(herramientas[i]) 
            if len(agotados) > 0:
                for a in agotados: # Si la lista de agotados tiene elementos, los muestro
                    print(f"La herramienta {a} está agotada.")
            else: # Si no se encuentra, es que no hay ningun elemento agotado
